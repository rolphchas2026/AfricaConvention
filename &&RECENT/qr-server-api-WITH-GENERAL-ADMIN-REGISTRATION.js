...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    nav h1 { background: linear-gradient(135deg, #667eea, #f093fb); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-size: 26px; font-weight: 700; }
    ...
    .hero h2 { font-size: 48px; margin-bottom: 20px; text-shadow: 0 4px 15px rgba(0,0,0,0.2); }
    .hero p { font-size: 18px; margin-bottom: 30px; opacity: 0.95; }
    
    .section { padding: 60px 40px; }
    .gallery-section { background: white; }
    
    .section h2 { color: #667eea; text-align: center; margin-bottom: 40px; font-size: 32px; animation: fadeInUp 0.8s ease-out 0.2s both; }
    .section p { text-align: center; color: #666; margin-bottom: 40px; font-size: 16px; }
    
    .gallery { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; max-width: 1200px; margin: 0 auto; }
    .gallery-item { cursor: pointer; border-radius: 15px; overflow: hidden; box-shadow: 0 5px 20px rgba(0,0,0,0.1); transition: all 0.3s; animation: slideUp 0.6s ease-out; }
    @keyframes slideUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
    
    .gallery-item:hover { transform: translateY(-15px); box-shadow: 0 15px 50px rgba(102,126,234,0.4); }
    .gallery-item img { width: 100%; height: 250px; object-fit: cover; display: block; }
    
    .modal { display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.7); align-items: center; justify-content: center; animation: fadeIn 0.3s; }
    ...
    .close-btn { position: absolute; top: 15px; right: 20px; font-size: 32px; font-weight: bold; cursor: pointer; color: #667eea; transition: all 0.3s; }
    ...
    .ticket-card:hover { transform: translateY(-10px); box-shadow: 0 15px 40px rgba(102,126,234,0.3); }
    .ticket-card h3 { color: #667eea; margin-bottom: 10px; font-size: 18px; }
    .ticket-card .price { font-size: 28px; font-weight: bold; color: #764ba2; margin-bottom: 15px; }
    .ticket-card button { width: 100%; padding: 12px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 25px; cursor: pointer; font-weight: bold; transition: all 0.3s; }
    ...
    .form-group input:focus, .form-group select:focus { outline: none; border-color: #667eea; box-shadow: 0 0 12px rgba(102,126,234,0.2); }
    .form-group button { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 14px; border: none; border-radius: 8px; cursor: pointer; font-weight: bold; }
    .form-group button:hover { box-shadow: 0 8px 20px rgba(102,126,234,0.4); }
    .msg { padding: 12px; margin-bottom: 15px; border-radius: 8px; text-align: center; }
    .msg.success { background: #d4edda; color: #155724; }
    .msg.error { background: #f8d7da; color: #c92a2a; }
    ...
    .contact-info { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 30px; max-width: 1200px; margin: 0 auto; }
    .contact-card { background: rgba(255,255,255,0.15); padding: 25px; border-radius: 15px; backdrop-filter: blur(10px); }
  </style>
</head>
<body>
  <nav>
  ...
  <div class="gallery-section">
    <h2>Event Gallery</h2>
    <p>Experience the African Convention 2026</p>
  ...
  <script>
    // Gallery with actual images from sysimages folder
    const galleryImages = [
      '/sysimages/image1.jpg',
      '/sysimages/image2.jpg',
      '/sysimages/image3.jpg',
      '/sysimages/image4.jpg'
    ];
    ...
    galleryImages.forEach((img, i) => {
      ...
      div.innerHTML = '<img src="' + img + '" alt="Gallery ' + (i+1) + '" style="width:100%; height:250px; object-fit:cover;">';
      div.onclick = () => openGallery(img);
    ...
    const ticketTypes = [
      { name: 'General Admin (Local)', price: '10,000', currency: 'TZS', icon: '👔' },
      { name: 'Foreigners (VIP)', price: '350', currency: 'USD', icon: '✈️' },
      { name: 'Youth', price: '200', currency: 'USD', icon: '🎓' },
      { name: 'Speaker', price: '300', currency: 'USD', icon: '🎤' },
      { name: 'Business', price: '250', currency: 'USD', icon: '💼' }
    ];
    ...
    ticketTypes.forEach(t => {
      ...
      card.innerHTML = '<div style="font-size: 40px; margin-bottom: 15px;">' + t.icon + '</div><h3>' + t.name + '</h3><div class="price">' + t.price + ' ' + t.currency + '</div>';
    ...
    async function registerAttendee() {
      ...
      try {
        const res = await fetch('/api/register', {
          ...
          body: JSON.stringify({ ticket_type: ticketType, name, email, phone, organization, title })
        });
        const result = await res.json();
        if (result.success) {
          msgDiv.innerHTML = '<div class="msg success">✅ Registration successful! Ticket ID: ' + result.ticket_id + '</div>';
          ...
          msgDiv.innerHTML = '<div class="msg error">❌ ' + result.error + '</div>';
        }
      } catch (err) {
        msgDiv.innerHTML = '<div class="msg error">❌ Error: ' + err.message + '</div>';
      }
    }
  </script>
</body>
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #f5f5f5 0%, #e8e8f0 100%); }
    
    .container { display: flex; flex-direction: column; height: 100vh; }
    
    header { background: linear-gradient(135deg, #667eea 0%, #764ba2 50%, #f093fb 100%); color: white; padding: 25px 40px; display: flex; justify-content: space-between; align-items: center; flex-shrink: 0; box-shadow: 0 8px 32px rgba(102,126,234,0.3); }
    header h1 { font-size: 28px; font-weight: 700; }
    header a { color: white; padding: 12px 24px; background: rgba(255,255,255,0.2); border-radius: 25px; cursor: pointer; text-decoration: none; transition: all 0.3s; backdrop-filter: blur(10px); }
    ...
    .tab-btn.active { color: #667eea; border-bottom-color: #667eea; font-weight: 600; background: linear-gradient(180deg, rgba(102,126,234,0.05) 0%, transparent 100%); }
    
    .content-wrapper { flex: 1; overflow-y: auto; padding: 40px; background: linear-gradient(135deg, #f5f5f5 0%, #e8e8f0 100%); }
    .tab-content { display: none; animation: fadeInUp 0.4s ease-out; }
    .tab-content.active { display: block; }
    @keyframes fadeInUp { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
    
    .stat-box { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 25px; border-radius: 16px; display: inline-block; margin-right: 20px; margin-bottom: 20px; min-width: 200px; text-align: center; box-shadow: 0 8px 25px rgba(102,126,234,0.3); transition: all 0.3s; }
    .stat-box:hover { transform: translateY(-5px); box-shadow: 0 12px 35px rgba(102,126,234,0.4); }
    .stat-number { font-size: 36px; font-weight: bold; }
    .stat-label { font-size: 13px; margin-top: 10px; opacity: 0.95; }
    
    table { width: 100%; border-collapse: collapse; margin-top: 20px; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.08); }
    th, td { padding: 14px 16px; text-align: left; border-bottom: 1px solid #f0f0f0; color: #333; }
    th { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; font-weight: 600; }
    tr:hover { background: #f9f9f9; }
    
    .badge { display: inline-block; padding: 6px 12px; border-radius: 12px; font-size: 11px; font-weight: bold; }
    .badge-approved { background: linear-gradient(135deg, #84fab0 0%, #8fd3f4 100%); color: #155724; }
    .badge-pending { background: linear-gradient(135deg, #fa709a 0%, #fee140 100%); color: #856404; }
    
    input { padding: 12px; border: 2px solid #ddd; border-radius: 8px; font-size: 14px; color: #333; transition: all 0.3s; }
    input:focus { outline: none; border-color: #667eea; box-shadow: 0 0 12px rgba(102,126,234,0.2); }
    
    button { padding: 11px 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s; box-shadow: 0 4px 12px rgba(102,126,234,0.3); }
    ...
    h3 { color: #667eea; margin-top: 30px; margin-bottom: 15px; font-weight: 600; }
    
    .info-msg { padding: 14px; margin-bottom: 15px; border-radius: 8px; font-weight: 500; }
    .info-msg.success { background: linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%); color: #155724; border-left: 4px solid #28a745; }
    .info-msg.error { background: linear-gradient(135deg, #f8d7da 0%, #f5c6cb 100%); color: #c92a2a; border-left: 4px solid #dc3545; }
    
    iframe { width: 100%; height: 700px; border: 2px solid #ddd; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); }
  </style>
</head>
<body>
  <div class="container">
